...
class PasswordLine():
    ...
    def valid(self):
        if not self.password:
            self.parse()
        return (self.min <= self.password.count(self.char) and
                self.password.count(self.char) <= self.max)
...
def part1(data):
    print(len([line for line in data if line.valid()]))


def part2(data):
    print(len([line for line in data if line.valid_part2()]))
